...
static float Euclidian_Cell_Distance_Estimate_Sqrd(const CELL source, const CELL dest)
{
...
static float Euclidian_Cell_Distance_Estimate(const CELL source, const CELL dest)
{
...
int FootClass::Find_Path_AStar(PathType* const resultPath, const CELL source, CELL dest, const int maxLen, const MoveType threshhold, const int threat) 
{
	...
	while (!open_list.empty()) {
		...
		for (FacingType facing = FACING_FIRST; facing < FACING_COUNT; ++facing) {
			...
			int cell_cost = Passable_Cell(adjacent_cell_id, facing, threat, threshhold);
			if (cell_cost)
			{
	...
	if (dest_result)
	{
		...
		resultPath->Cost = dest_result->cost_from_start;
